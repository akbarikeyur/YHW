...
//    }
    ...
    @objc open func showPickerDateAndTime(title: String?, selected: Date?, min: Date?, max: Date?, completion:DPPickerDateCompletion?) {
        self.showPicker(title: title, picker: { (picker) in
            picker.date = selected ?? Date()
            picker.minimumDate = min
            picker.maximumDate = max
    ...
    @objc open func showPicker(title: String?, view: UIView, completion:DPPickerCompletion?) {
        ...
        if view.bounds.size.width > (UIScreen.main.bounds.size.width-20) {
            var newFrame = view.frame
            newFrame.size.width = (UIScreen.main.bounds.size.width-20)
            view.frame = newFrame
        }
    ...
    @objc internal func pickerClose(_ sender: Any) {
