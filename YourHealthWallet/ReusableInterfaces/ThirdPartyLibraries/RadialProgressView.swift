...
//
//
//  Created by Vishal Nandoriya on 20/07/18.
...
class RadialProgressView: UIView {
    ...
    @IBInspectable var isBackgroundImage:Bool = true{
        didSet {updateLayerProperties()}
    }
    ...
    func createPie(){
        ...
        if ringLayer == nil {
            ...
            let rectangle = bounds.insetBy(dx: inset, dy: inset)
            let path = UIBezierPath(ovalIn: rectangle)
            ringLayer.transform = CATransform3DMakeRotation(CGFloat((-Double.pi/2)), 0, 0, 1)
        ...
        if percentageLayer == nil {
            ...
            percentageLayer.font = UIFont(name: "HelveticaNeue-Light", size: 10)
            percentageLayer.frame = CGRect(x: 0, y: 0, width: bounds.size.width, height: percentageLayer.fontSize)
