...
protocol GetRunDayTypeDelegate:class {
    func getRunDayType(selectedDaysType:Int)
}
class RunFirstCell: TableViewCell {
    ...
    weak var delegate: GetRunDayTypeDelegate?
    ...
    func setCalculateRunData(firstCellWorkouts:[HKWorkout]?){
        self.chartXValues = []
        self.chartYValues = []
    ...
    @objc func handleTap(gestureRecognizer: UIGestureRecognizer) {
        DPPickerManager.shared.showPicker(title: nil, selected: daysTypes[selectedDaysType], strings: daysTypes) { (value, index, cancel) in
            if cancel == false && value != nil{
                ...
                self.delegate?.getRunDayType(selectedDaysType: index)
    ...
    func setTodayBarGraphValue() {
        ...
        if self.chartXValues.count > 0{
            ...
            dataSet.colors.append(ColorType.FitnessRunPinkColor.value) //bar line color
            ...
            let marker = XYMarkerView(color: FitnessTrackerRunPinkColor,
...
extension RunFirstCell : IAxisValueFormatter {
