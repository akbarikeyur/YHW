...
//  YourHealthWallet
//
//  Created by Vishal Nandoriya on 3/13/18.
...
protocol GetWalkDayTypeDelegate:class {
    func getWalkDayType(selectedDaysType:Int)
}

class WalkFirstCell: TableViewCell {
    ...
    weak var delegate: GetWalkDayTypeDelegate?
    ...
    func setCalculateWalkData(firstCellWorkouts:[HKWorkout]?){
        self.chartXValues = []
        self.chartYValues = []
    ...
    @objc func handleTap(gestureRecognizer: UIGestureRecognizer) {
        DPPickerManager.shared.showPicker(title: nil, selected: daysTypes[selectedDaysType], strings: daysTypes) { (value, index, cancel) in
            if cancel == false && value != nil{
                ...
                self.delegate?.getWalkDayType(selectedDaysType: index)
    ...
    func setTodayBarGraphValue() {
        ...
        if self.chartXValues.count > 0{
            ...
            let marker = XYMarkerView(color: OrangeColor,
...
extension WalkFirstCell : IAxisValueFormatter {
